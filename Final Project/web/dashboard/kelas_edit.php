...
$queryMK = mysqli_query($koneksi, "SELECT * FROM matakuliah");
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid">
                    <h3 class="mt-4">Edit Kelas</h3>
                    <div class="card mb-4">
                        <form form action="dashboard.php?act=editKelas&id_kelas=<?= $id_kelas; ?>" id="buat" method="POST">
                            <div class="form-group ml-4 py-3 mb-0">
                                <label for="namaKelas">Nama Kelas</label>
                                <select id="namaKelas" name="namaKelas" id="status" class="form-control col-sm-3">
                                    <?php if ($queryMK) : ?>
                                        <?php while ($matakuliah = mysqli_fetch_assoc($queryMK)) : ?>
                                            <option value="<?= $matakuliah['id_mk'] ?>" <?php echo ($krs['id_mk'] == $matakuliah['id_mk']) ? 'selected' : ''; ?>><?= $matakuliah['matakuliah'] ?></option>
                                        <?php endwhile ?>
                                    <?php endif ?>
                                </select>
                            </div>
